...
class APIService {
    ...
    // 验证API密钥
    static func validateAPIKey(baseURL: String, apiKey: String, completion: @escaping (Result<[AIModel], APIError>) -> Void) {
        // 构建URL
        let modelsEndpoint = baseURL.hasSuffix("/") ? "\(baseURL)models" : "\(baseURL)/models"
        ...
        print("📤 Request Headers: \(request.allHTTPHeaderFields?.description ?? "None")")
            ...
            case 200...299:
                ...
                if let data = data {
                    do {
                        ...
                        print("📋 Available Models: \(modelResponse.data.map { $0.id }.joined(separator: ", "))")
