    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                // 应用统计信息
                HStack(spacing: 40) {
                    statsView(count: keyStore.apiKeys.count, title: "已保存密钥", icon: "key")
                    
                    statsView(count: Set(keyStore.apiKeys.map { $0.provider }).count, title: "服务提供商", icon: "building.2")
                }
                .padding(.vertical, 20)
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    HStack(spacing: 20) {
                        actionButton(title: "添加新密钥", icon: "plus.circle", action: {
                            showingAddSheet = true
                        })
                        
                        if !keyStore.apiKeys.isEmpty {
                            actionButton(title: "查看所有密钥", icon: "list.bullet", action: {
